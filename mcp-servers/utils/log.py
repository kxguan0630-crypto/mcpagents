    ...
    def emit(self, record):
        try:
            ...
            if self.current_date_str != date_str:
                ...
                log_file_path = os.path.join(date_dir, "server.log")
...
def setup_logger():
    ...
    RUNTIME_PATH = settings.LOG_FILE_PATH
    ...
    # # 4. 日志文件路径
    # # 子目录的创建。
    # log_file = os.path.join(date_dir, 'server.log')
    ...
    # except Exception as e:
    #     # 如果创建失败，打印详细错误
    #     print(f"CRITICAL: Failed to create log file at {log_file}. Error: {e}")
    #     print(f"Attempting to fallback to console only...")
